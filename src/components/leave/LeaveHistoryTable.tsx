...
const STATUS_UI: Record<LeaveStatus, { label: string; className: string }> = {
  pending: {
    label: "Pending",
...
export function LeaveHistoryTable({
  ...
  const pageSize = clientPaginate ? pageSizeProp : server ? server.pageSize : leaves.length || 1;
  ...
  const visibleLeaves = clientPaginate ? leaves.slice(sliceStart, sliceStart + pageSize) : leaves;

  const totalPagesServer = server ? Math.max(1, Math.ceil(server.total / server.pageSize)) : 1;
  const safePageServer = server ? Math.min(Math.max(1, server.page), totalPagesServer) : 1;
  const sliceStartServer = server ? (safePageServer - 1) * server.pageSize : 0;

  /** Out-of-range page or refetch gap: avoid flashing “empty” while totals say otherwise. */
  const bodySkeleton = isLoading || Boolean(server && server.total > 0 && leaves.length === 0);

  const showEmptyRow = !bodySkeleton && (server ? server.total === 0 : leaves.length === 0);
  ...
  const renderEmpty = () => {
    if (emptyDescription) {
      return (
        <>
          <CalendarDays className={`mx-auto mb-2 h-10 w-10 ${emptyIcon}`} aria-hidden />
      ...
      <>
        ...
        <p className="text-sm">
          ...
          {showRequestLink && (
            <>
              {" "}
              <Link to="/leave" className="font-medium text-indigo-600 hover:text-indigo-700">
  ...
  return (
    <div className="space-y-4">
      {showSectionHeader && (
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500">Your submitted requests and their status.</p>
          </div>
          {showRequestLink && (
            <Link
              to="/leave"
        ...
        <table className="min-w-full text-left text-sm">
          ...
          <tbody>
            {bodySkeleton ? (
              ...
              <tr>
                <td colSpan={6} className={`px-4 py-12 text-center ${emptyWrap}`}>
                  {renderEmpty()}
                </td>
              </tr>
            ) : (
              visibleLeaves.map((row) => (
                <tr key={row._id} className={rowClass}>
                  <td className="px-4 py-3 align-top">
                    <div className={`font-medium ${datePrimary}`}>{formatDate(row.fromDate)}</div>
                    {row.days === "multiple" && row.toDate && row.toDate !== row.fromDate && (
                      <div className={`text-xs ${dateSecondary}`}>→ {formatDate(row.toDate)}</div>
                    )}
                  </td>
                  <td className="px-4 py-3 align-top tabular-nums">{leaveDurationLabel(row)}</td>
                  <td className="px-4 py-3 align-top">
                  ...
                  <td className={`max-w-xs px-4 py-3 align-top ${reasonCls}`}>{row.reason}</td>
                  <td className={`max-w-[200px] px-4 py-3 align-top text-xs ${noteCls}`}>
      ...
      {server && !bodySkeleton && !isLoading && server.total > 0 && (
        <div
        ...
        >
          <p className="text-sm tabular-nums">
            Showing{" "}
            <span className={`font-medium ${slate ? "text-slate-900" : "text-gray-900"}`}>
              {sliceStartServer + 1}–{Math.min(safePageServer * server.pageSize, server.total)}
            </span>{" "}
            of{" "}
            <span className={`font-medium ${slate ? "text-slate-900" : "text-gray-900"}`}>
              {server.total}
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              onClick={() => server.onPageChange(Math.max(1, safePageServer - 1))}
            ...
            <button
              ...
              onClick={() => server.onPageChange(Math.min(totalPagesServer, safePageServer + 1))}
              className={`inline-flex items-center gap-1 rounded-lg border px-3 py-1.5 text-sm font-medium transition disabled:cursor-not-allowed disabled:opacity-40 ${
                slate
                  ? "border-slate-200 bg-white text-slate-800 hover:bg-slate-50"
                  : "border-gray-200 bg-white text-gray-800 hover:bg-gray-50"
        ...
        >
          <p className="text-sm tabular-nums">
            ...
            <span className="font-medium text-slate-900">
              {sliceStart + 1}–{Math.min(sliceStart + pageSize, leaves.length)}
            </span>{" "}
            of <span className="font-medium text-slate-900">{leaves.length}</span>
          </p>
          <div className="flex items-center gap-2">
            <button
              type="button"
            ...
            <button
              ...
              onClick={() => setPage((p) => Math.min(totalPagesClient, p + 1))}
              className={`inline-flex items-center gap-1 rounded-lg border px-3 py-1.5 text-sm font-medium transition disabled:cursor-not-allowed disabled:opacity-40 ${
                slate
                  ? "border-slate-200 bg-white text-slate-800 hover:bg-slate-50"
                  : "border-gray-200 bg-white text-gray-800 hover:bg-gray-50"
